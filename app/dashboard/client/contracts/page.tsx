...
function ClientContractsContent() {
  ...
  const completedMilestones = (c: Contract) =>
    c.milestones?.filter((m) => m.status === "Completed").length ?? 0;
  const totalMilestones = (c: Contract) => c.milestones?.length ?? 0;

  return (
    <main className="min-h-screen bg-[#08080d] text-white px-4 py-8">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-1">Contracts</h1>
        <p className="text-[#9090aa] text-sm mb-6">Manage your active and past contracts with freelancers.</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-[#9090aa]">
            ...
            <p className="text-sm">Contracts are created when you accept a proposal.</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#9090aa]">
                      Freelancer: <span className="text-white">{c.freelancer?.fullName || "—"}</span>
                      {" · "}
                      <span className="font-medium text-white">${c.amount?.toLocaleString()}</span>
                      {" · "}
                      {totalMilestones(c) > 0 && (
                        <span>{completedMilestones(c)}/{totalMilestones(c)} milestones</span>
                      )}
                    </p>
                  </div>
                  <Link
                    href={`/dashboard/client/contracts/${c._id}`}
...
export default function ClientContractsPage() {
  return <ProtectedRoute allowedRoles={["Client"]}><ClientContractsContent /></ProtectedRoute>;
}
